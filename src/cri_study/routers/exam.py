...
from schemas.exam import SubmitExamRequest
...
async def exam_page(
    ...
    if not exam:
        return templates.TemplateResponse(
            "error.html", {"request": request, "message": "試験が見つかりません"}
        )
...
async def submit_exam_form(
    ...
    try:
        ...
        return RedirectResponse(
            url=f"/exam/{exam_id}/result/{exam_result.id}", status_code=303
        )

    except ValueError as e:
        return templates.TemplateResponse(
            "error.html", {"request": request, "message": str(e)}, status_code=400
        )
...
async def exam_result_page(
    ...
    if not exam_result:
        return templates.TemplateResponse(
            "error.html", {"request": request, "message": "結果が見つかりません"}
        )
